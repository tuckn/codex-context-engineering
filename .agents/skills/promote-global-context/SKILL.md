...
この Skill は `~/.codex-context` への bridge script を `scripts/` に同梱している。local plugin が使えない Codex 環境でも、Skill directory だけで dry-run と write を実行できる。
...
`.codex-context/` 全体を盲目的に読まない。対象は、ユーザー指定の source、現在の canonical session note、`.codex-context/working-context.md`、relevant decision records、または `review-decisions` が示した候補に絞る。
...
- `type`: global store 内での artifact kind。repo-local の `decision` / `workingContext` と区別する。
...
7. script に渡す body file を用意する。
   ...
   - command log や一時検証出力は `.local/` に置く。
...
13. promoted item が durable repo state を変える場合、`.codex-context/working-context.md` の更新または repository decision record 作成の検討。

## Metadata updates after promotion

global write が成功した場合だけ、repo-local metadata を更新する。
...
repository root からは Skill 内に同梱された script を直接指定して実行する。Skill directory に移動済みの場合は `python3 scripts/promote_context.py ...` でよい。

Candidate example:

```bash
python3 .agents/skills/promote-global-context/scripts/promote_context.py \
  ...
  --source-repo notes \
  --dry-run \
  --log .local/context-bridge/promote-candidate-dry-run.log
```
...
python3 .agents/skills/promote-global-context/scripts/promote_context.py \
  ...
  --body-file .codex-context/decisions/DR-0008-use-explicit-skill-bridge-for-global-context.md \
  --source-repo notes \
  --write \
  --log .local/context-bridge/promote-decision-write.log
```
...
python3 .agents/skills/promote-global-context/scripts/promote_context.py \
  ...
  --source-repo notes \
  --write \
  --log .local/context-bridge/promote-working-context-write.log
```
