...
repository root からは Skill 内に同梱された script を直接指定して実行する。Skill directory に移動済みの場合は `python3 scripts/import_context.py ...` でよい。

推奨 command:

```bash
python3 .agents/skills/import-global-context/scripts/import_context.py \
