...
`.codex-context/decisions/DR-NNNN-<decision-title-slug>.md`

- `DR-NNNN` は four-digit sequence number。
- 既存の `.codex-context/decisions/DR-*.md` filenames を確認し、次に利用可能な number を使用。
...
- `promotionStatus`: silver artifact である repo-local decision から gold global context への昇格状態。`pending`、`partial`、`promoted`、`no-action` のいずれかを使う。
- `promotedTo`: 昇格先の paths。未昇格なら `[]`。`~/.codex-context/decisions/DR-G-*.md`、`.codex-context/global-context/decisions/*.md` などを列挙する。
...
- decision が `AGENTS.md`、`README.md`、specs、design docs、operation docs、`.codex-context/working-context.md`、reusable skills を更新すべき場合、`Notes` への記載。
